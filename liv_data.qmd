...
based on the LIterate Programmign paradigm of Knuth.
...
Define a function to do a TOST test with bounds of about 10% of the group mean

```{r}
TOST_10 <- function(data, deps, group) {
  # Set the size of the equivalence bounds at 10% of the combined mean 
  # of all cushions in both groups.
  eqbound_width = 0.1 * mean(pull(data, {{deps}}))
  # Then do a TOST using that bound size (half above and half below).
  TOSTER::dataTOSTtwo(data = data, deps = {{deps}}, group = {{group}},
                            eqbound_type = "raw",
                            low_eqbound = -eqbound_width / 2,
                            high_eqbound = eqbound_width / 2)
}
```
...
result_list <- liv_data %>% 
  ...
  group_map( 
    ~ TOST_10(data = .x, deps = value, group = foam) # bounds at 10% of mean
    # SMD version, using Cohen's d for moderate effect size 
    #   ~ TOSTER::dataTOSTtwo(data = .x, deps = value, group = foam,
    #                         eqbound_type = "SMD",
    #                         low_eqbound = -0.5,
    #                         high_eqbound = 0.5)
   ) 
...
result_names <- liv_data %>% 
    ungroup() %>% # remove the old groups first
    group_by(var, level) %>% group_keys() %>% pull(var,level)

names(result_list) <- paste(result_names, names(result_names))
