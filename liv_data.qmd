...
```{r}
# Cleaner & better way to do the analysis, passing a formula to t_TOST,
# with data = .x, in the same way as is done with t.test()
result_list <- liv_data %>% 
    ungroup() %>% # remove the old groups first
    group_by(var, level) %>% # then regroup
    group_map(~ TOSTER::t_TOST(value ~ foam, data = .x, 
                               paired = FALSE, 
                               eqb = mean(.x$value)/20) 
              )
